    ...
    private void OnPlaceBuildingPerformed(InputAction.CallbackContext context)
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            if(pos.x >= 0 && pos.x < gridSystem.gridSize.x && pos.y >= 0 && pos.y < gridSystem.gridSize.y)
                ...
                {
                    Vector3 place = new Vector3(gridSystem.GetCell(pos).cellPosition.x, prefab.buildingPrefab.GetComponent<Renderer>().bounds.size.y / 2, gridSystem.GetCell(pos).cellPosition.y);
